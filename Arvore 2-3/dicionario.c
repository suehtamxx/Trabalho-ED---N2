...
void imprimir_unid_BB(arv_ingles *ingles, int unid)
{
    if (ingles != NULL)
    {
        unidade *atual;
        ...
        imprimir_unid_BB(ingles->esq, unid);
        imprimir_unid_BB(ingles->dir, unid);
    }
}
void imprimir_unid_B3(arv_ptbr *portugues, int unid)
{
    if (portugues != NULL)
    {
        unidade *atual = portugues->info1.ingles->info.l_unidade;
        while (atual != NULL)
        {
            if (atual->unidade == unid)
            {
                printf("Portugues: %s\n", portugues->info1.ptbr);
                imprimir_unid_BB(portugues->info1.ingles, unid);
            }
        ...
        {
            atual = portugues->info2.ingles->info.l_unidade;
            while (atual != NULL)
            {
                if (atual->unidade == unid)
                {
                    printf("Portugues: %s\n", portugues->info2.ptbr);
                    imprimir_unid_BB(portugues->info2.ingles, unid);
                }
