...
void buscar_palavras(arv_ptbr *raiz, char *palavra, char *caminho, int *encontrou)
{
    if(raiz != NULL)
    {
        if (strcmp(raiz->info1.ptbr, palavra) == 0)
        {
            *encontrou = 1;
            strcat(caminho, raiz->info1.ptbr);    
        }
        
        else if (raiz->nInfos == 2 && strcmp(raiz->info2.ptbr, palavra) == 0)
        {
            *encontrou = 1;
            strcat(caminho, raiz->info2.ptbr);
        }

        if (*encontrou == 0)
        {
            if (strcmp(palavra, raiz->info1.ptbr) < 0)
            {
                strcat(caminho, raiz->info1.ptbr);
                strcat(caminho, " -> ");
            ...
            else if (raiz->nInfos == 1 || strcmp(palavra, raiz->info2.ptbr) < 0)
            {
                strcat(caminho, raiz->info2.ptbr);
                strcat(caminho, " -> ");
                buscar_palavras(raiz->cen, palavra, caminho, encontrou);
            }
...
void preencher_matriz_arvore(arv_ptbr *raiz, char palavras[][100], int *posicao)
{
    if (raiz != NULL)
    {
        ...
        strcpy(palavras[*posicao], raiz->info1.ptbr);
        (*posicao)++;         
        
        if (raiz->nInfos == 2)
        {
            strcpy(palavras[*posicao], raiz->info2.ptbr);
            (*posicao)++;  
            preencher_matriz_arvore(raiz->dir, palavras, posicao); 
        }
        preencher_matriz_arvore(raiz->cen, palavras, posicao);
    }
}
